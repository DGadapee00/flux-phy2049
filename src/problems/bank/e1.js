...
export default [
  ...
  problem({
    ...
    hints: ['|a| = √(aₓ² + a_y²)', 'tan⁻¹(a_y/aₓ) only gives the right angle in quadrants I and IV. Check which quadrant the vector is in.'],
    steps: ($, f) => [`|a| = √(${f($.ax)}² + ${f($.ay)}²) = ${f($.A)}`, `θ = ${f($.th)}° (quadrant from the signs of the components)`],
    sim: {
      scenario: 'axes',
      setup(s, $) {
  ...
  problem({
    ...
    parts: [num('ax', ($) => $.ax, '', { label: 'aₓ', abs: 0.02 }), num('ay', ($) => $.ay, '', { label: 'a_y', abs: 0.02 })],
    steps: ($, f) => [`aₓ = A cos θ = ${f($.ax)}`, `a_y = A sin θ = ${f($.ay)}`],
    sim: {
      scenario: 'axes',
      setup(s, $) {
  ...
  problem({
    ...
    hints: ['a·b = aₓbₓ + a_y b_y = |a||b| cos φ'],
    steps: ($, f) => [`a·b = ${f($.dot)}`, `cos φ = a·b / (|a||b|) → φ = ${f($.phi)}°`],
    sim: {
      scenario: 'axes',
      setup(s, $) {
  ...
  problem({
    ...
    parts: [num('R', ($) => $.R, '', { label: '|R|' }), num('th', ($) => $.th, '°', { label: 'θ_R', abs: 0.6, tol: 0.005, wrap: 360 })],
    hints: ['Add components, not magnitudes.'],
    steps: ($, f) => [`Rₓ = ${f($.Rx)}, R_y = ${f($.Ry)}`, `|R| = ${f($.R)} at ${f($.th)}°`],
    sim: null,
  ...
  problem({
    ...
    hints: ['(a × b)_z = aₓb_y − a_y bₓ', 'Right-hand rule: x̂ × ŷ = +ẑ'],
    steps: ($, f) => [`(a × b)_z = ${f($.ax)}·${f($.by)} − ${f($.ay)}·${f($.bx)} = ${f($.cz)}`],
  ...
  problem({
    ...
    steps: () => ['a·b = |a||b| cos φ: positive for φ < 90°, zero at 90°, negative for φ > 90°.'],
    sim: {
      scenario: 'axes',
      setup(s, $) {
        s.a = { x: 1, y: 0, z: 0 };
  ...
  problem({
    ...
    steps: ($, f) => [`N = |q|/e = ${f($.q)} / 1.6×10⁻¹⁹ = ${f($.N)}`, 'A positive object lost electrons; a negative object gained them.'],
  ...
  problem({
    ...
    steps: ($, f) => [`After 1–2: each has (q₁ + q₂)/2 = ${f($.a12 * 1e6)} μC`, `After 2–3: each has (${f($.a12 * 1e6)} + q₃)/2 = ${f($.final3 * 1e6)} μC`],
  ...
  problem({
    ...
    steps: () => ['The paper polarizes: charge opposite to the comb shifts to the near side, so the attraction beats the repulsion (force ∝ 1/r²). This happens for either sign.'],
  ...
  problem({
    ...
    steps: ($, f) => [`F = k|q₁q₂|/r² = ${f($.F)} N`, 'Like signs repel; opposite signs attract.'],
    sim: {
      scenario: 'pair-repel',
      setup(s, $) {
  ...
  problem({
    ...
    hints: ['Find each force separately with its direction, then add them as signed x components.'],
    steps: ($, f) => [`From q₁: ${f($.F1)} N`, `From q₃: ${f($.F3)} N`, `Net: ${f($.Fx)} N`],
    sim: {
      scenario: 'three',
      setup(s, $) {
  ...
  problem({
    ...
    hints: ['k q₁ q/x² = k q₂ q/(d − x)². The third charge cancels out.'],
    steps: ($, f) => [`x/(d − x) = √(q₁/q₂) → x = d / (1 + √(q₂/q₁)) = ${f($.x)} m`],
    sim: {
      scenario: 'three',
      setup(s, $) {
  ...
  problem({
    ...
    steps: ($, f) => [`Fₓ = ${f($.Fx)} N, F_y = ${f($.Fy)} N`, `|F| = ${f($.F)} N at ${f($.th)}°`],
    sim: {
      scenario: 'three',
      setup(s, $) {
  ...
  problem({
    ...
    parts: [num('Fe', ($) => $.Fe, 'N', { label: 'F_E' }), num('ratio', ($) => $.ratio, '', { label: 'F_E / F_G' })],
    steps: ($, f) => [`F_E = ke²/r² = ${f($.Fe)} N`, `F_G = G mₑ m_p / r² = ${f($.Fg)} N`, `Ratio = ${f($.ratio)} (independent of r)`],
  ...
  problem({
    ...
    hints: ['Each sphere: T cos θ = mg and T sin θ = F_E, so F_E = mg tan θ.', 'Separation r = 2L sin θ'],
    steps: ($, f) => [`F_E = mg tan θ = ${f($.F)} N`, `r = 2L sin θ = ${f($.r)} m`, `q = r √(F_E/k) = ${f($.q)} C`],
  ...
  problem({
    ...
    steps: ($, f) => [`F ∝ q₁q₂/r² → ${f($.a)}·${f($.b)}/${f($.c)}² = ${f($.factor)}`],
