...
- **Tests.** `npm test` also runs the progress, `pickSet`, principle-step and `pickInterleave` checks. `scripts/smoke.mjs` loads every problem that has a lab setup into the running app. It walks a Circuits problem through the principle step and blind mode (principle gate, wrong answer, feedback, right answer, lab agrees, recorded) and runs a practice exam to its results.
