...
| `src/problems/principles.js` | 28 governing principles (statement, cue, conserved quantity) and a `[primary, …secondary]` tag list for every template, folded in as `tpl.principles`. Drafted and accepted for FLUX; an instructor review comes first if it is adopted class-wide. The check requires every template tagged with defined principles |
| `src/problems/fading.js` | Fading worked examples: `guidanceFor` (full example → faded steps → none, from chapter readiness), `exampleSeed`, `workedExample` |
| `src/problems/principle-step.js` | The "which principle decides this?" step: who gets it (`asksPrinciple`), the four seeded options (`principleOptions`), grading |
...
- **Principle step.** Every practice attempt (not an exam, and not "same problem, new numbers") whose primary principle is not a tool-kit entry (vector algebra, charge density) opens by asking which principle decides it. That covers 247 of 258 templates. `principleOptions` in `src/problems/principle-step.js` offers the primary and three distractors, chosen from a list of principles commonly confused with it and seeded by the attempt. It never offers one of the template's secondary principles, since those are also true, and never a near-twin of the primary (`TWINS`: energy/potential, ohm/power, Faraday/inductance, and so on). The parts stay hidden until the pick. A wrong or "not sure" pick shows the principle's statement and cue and makes the attempt not clean. The progress record counts `pAsk`/`pOk`. The solution names the deciding principle, the ones the problem also uses, and the conserved quantity. After a missed attempt (a wrong first check, or the solution opened), the panel asks what went wrong: applying one of the problem's principles, seeing which idea to use, a slip, or misreading the question. The answer is kept in `gaps`.
- **Fading worked examples.** `guidanceFor` in `src/problems/fading.js` decides each practice attempt's help from the chapter's readiness: how many of its numeric and derivation problems the student has solved right on the first check (no hints, no peek, principle named), counting `clean` and guided (`gOk`) solves. At readiness 0 the attempt shows a complete worked example of the same template from a different seed (`exampleSeed` makes sure the numbers differ). At readiness k it shows the first `steps − k` steps of the student's own solution, and none once k reaches the step count. Concept questions, "same problem, new numbers" and templates already solved get no help. A guided solve is recorded with `guided`/`rightFirst`: it adds to `gOk` but is never clean, so spaced review still brings it back soon.
- **Principles most often missed.** `progress.principleMisses(PROBLEMS)` adds up, per principle, the misnamed principle-step picks (`pAsk − pOk`, charged to the problem's primary), the "misapplied X" gap answers (charged to X), and "didn't see which idea" (charged to the primary). Slips and misreadings are counted separately. The list shows the top three, each with a **Practise** session: up to five of that principle's problems from any exam, via `pickSet`. Mixed sets pass `pickSet` a `weight` that moves those principles' problems ahead within each chapter.
- **Interleaving.** In a session opened from the chapter list, every third problem along the path is followed by one from an earlier chapter (`pickInterleave` in `progress.js`). It picks from problems already met: the most overdue first, otherwise the weakest one not tried in the last 12 hours. Nothing is added for a student with no history. The problem is flagged "Review from earlier", and Next returns to the path. Sessions keep a position (`session.pos`), so a problem inserted here can also appear earlier in the same list.
- **Group by principle.** The list's **Group by** chips switch between chapter and principle. The principle view orders principles as `principles.js` lists them, shows each one's statement, mastery and principle-step tally, and folds the same principle's problems from earlier exams into "From earlier exams". Opening a problem from this view starts a session in principle order, with no interleaving.
...
- **Tests.** `npm test` also runs the progress, `pickSet` (with weights), principle-step, `pickInterleave`, principle-miss and fading checks, and builds a worked example for every working template, checking that its numbers differ from the student's. It also runs every predict-first experiment in `src/data/predictions.js` from its preset through the lab's physics, and fails if the lab disagrees with the experiment's `expect` (and so with its explanation). `scripts/smoke.mjs` loads every problem that has a lab setup into the running app. It walks a predict-first experiment on the Capacitor lab (predict, change, graded, undo), and a Circuits problem through the principle step and blind mode (principle gate, wrong answer, feedback, right answer, lab agrees, recorded) and runs a practice exam to its results.
