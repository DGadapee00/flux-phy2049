...
3. **Answer and check.** Numbers accept `2.5e-6`, `2.5 × 10^-6`, or `2.5 μC` — a typed unit is converted, and one of the wrong kind is pointed out. A wrong part says why when it can (sign, power of ten, factor of 2, wrong quadrant), and a wrong multiple-choice pick explains what is wrong with that choice. Every problem has hints, from the idea to the equation to the step people miss, and the worked solution puts the numbers into each formula with their units before the result. Sketch parts are compared with a rubric.
